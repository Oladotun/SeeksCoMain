...
@section('content')
    ...
    <div class="filter nice-scroll">
        <form method="GET" action="{{ route('page.category.state.city', ['category_slug' => $category->category_slug, 'state_slug' => $state->state_slug, 'city_slug' => $city->city_slug]) }}">
            ...
            <div class="filter__btns">
                ...
                <a class="btn btn-outline-secondary filter__reset__btn" href="{{ route('page.category.state.city', ['category_slug' => $category->category_slug, 'state_slug' => $state->state_slug, 'city_slug' => $city->city_slug]) }}">{{ __('theme_directory_hub.filter-link-reset-all') }}</a>
            </div>
        </form>
        <hr>
    ...
    <section class="listing nice-scroll">
                ...
                @foreach($children_categories as $children_categories_key => $children_category)
                    <div class="col-12 col-md-6 col-lg-3 pr-0">
                        <div class="categories__item categories__item_sm">
                            <a href="{{ route('page.category.state', ['category_slug' => $children_category->category_slug, 'state_slug' => $state->state_slug]) }}">
        ...
        <div class="listing__text__top">
            <div class="listing__text__top__left">
                <h5>{{ __('frontend.category.city.sub-title-1', ['category_name' => $category->category_name, 'city_name' => $city->city_name, 'state_name' => $state->state_name]) }}</h5>
        ...
        @if($all_item_cities->count() > 0)
            <div class="listing__text__top">
                <div class="listing__text__top__left">
                    <h5>{{ __('frontend.category.state.sub-title-2', ['category_name' => $category->category_name, 'state_name' => $state->state_name]) }}</h5>
                </div>
            </div>
